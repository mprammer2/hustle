  ...
  ctx->spawnTask(CreateTaskChain(
    ...
    CreateLambdaTask([this](Task *internal) {
      ...
      for (std::size_t chunk_index = 0;
        ...
        auto val = agg_col->chunk(chunk_index)->data()->GetValues<int64_t>(1,
                                                                           0);
...
void HashAggregate::Finish(Task *ctx) {
  ...
  for (int i = 0; i < group_by_cols_.size(); i++) {
    ...
    switch (group_by_type_id) {
      ...
      case arrow::Type::INT64 : {
        ...
        auto status = builder.Reserve(tuple_map->size());
        assert(status.ok());
  ...
  // 4. Free all pointers
  // TODO: Refactor this to modern C++ using shared_ptr...
  switch (kernel) {
    case SUM:
    case COUNT: {
      delete ((HashMap *) (global_map));
      break;
    }
    case MEAN: {
      delete ((MeanHashMap *) (global_map));
      break;
    }
    default:
      assert(false);
  ...
  for(auto it : count_maps){
    delete it;
  }
  for(auto it : value_maps){
    delete it;
  }
